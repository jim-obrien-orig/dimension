...
Copy the `config.toml` from the exampleSite folder into your Hugo site's root folder. Change the fields as needed. Add or delete social media by following the examples in the file. You may need to look up the [font-awesome](http://fontawesome.io/) icon names. The icon field should be filled out with the font-awesome collection prefix (like 'fab' for the Brands collection) *and* the "fa-" prefix. The icon field for Twitter would be 'fab fa-twitter'.

You can change the logo as well with font-awesome icons. The default is set to `far fa-gem`.
To use an image as logo, remove the `logo = "far fa-gem"` line and replace it with `logoimage = "/path/to/image.png"` and place your image at that location in the `static` folder. The image should be in square format and at least 86*86 pixels for best viewing experience.
